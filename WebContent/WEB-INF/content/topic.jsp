...
<link href="${pageContext.request.contextPath}/css/topic.css"
		rel="stylesheet" type="text/css" />
...
<script type="text/javascript">
 $(function() {
  /* 记录滚动条的位置 */
  /* 获取滚动条的位置 */
  var scrollCookie = Cookies.get("scrollCookie_topic"+"${topic.id}${user.id}");
  if (scrollCookie != null && scrollCookie != "") {
   $("html,body").animate({
    scrollTop : scrollCookie + "px"
   }, 1000);
  } else {
   Cookies.set("scrollCookie_topic"+"${topic.id}${user.id}", $(document).scrollTop("0px"));
  }
  $(window).scroll(function() {
   Cookies.set("scrollCookie_topic"+"${topic.id}${user.id}", $(document).scrollTop());
  });
 });
</script>
</head>
<body>
		...
		<div class="user_info_panel">
				...
				<div class="selfinfo">
						<div class="name">
								<a
										href="${pageContext.request.contextPath}/user/personal/${topic.user.id}">
										${topic.user.name} </a> <br>
						</div>
						<div class="contact">
								<span id="followSpan"> <c:if test="${isFollowed eq true}">
												<a
														href="${pageContext.request.contextPath}/user/topic/notFollow"
														class="notFollow"> <i class="icon-minus" id="icon"></i><i
														id="text" style="font-style: normal;">取消关注</i>
										</c:if> <c:if test="${isFollowed ne true}">
												<a
														href="${pageContext.request.contextPath}/user/topic/follow"
														class="follow"> <i class="icon-plus" id="icon"></i><i
														id="text" style="font-style: normal;">关注</i>
												</a>
										</c:if>
		...
		<div class="notifying_panel" id="notifying_panel">
				<form
						...
						<textarea id="privMessContent" rows="7" cols="30"
								class="notifying_content" name="content"></textarea>
						<br> <input type="submit" value="发送私信" id="sendPrivMess">
		...
		<div class="show_topic" id="showTopic">
				...
				<c:set var="isFavourite" value="false"></c:set>
				...
				<div class="comment_form_panel">
						<form action="" method="post" class="comment_form">
								...
								<input type="submit" value="发表评论"> <input type="reset"
										value="收起" class="hiddenCommentForm">
						</form>
				</div>
				...
				<c:forEach items="${topic.comments}" var="comment">
						<div class="comment_panel">
								<p>${comment.user.name }
										: ${comment.content }<sub>&nbsp;&nbsp;${comment.date}</sub> <a
								...
								<c:forEach items="${comment.replies}" var="reply">
										<p>${reply.user.name}&nbsp;回复&nbsp;${comment.user.name}
												: ${reply.content }<sub>&nbsp;&nbsp;${reply.date}</sub> <a
										...
										<div class="reply_double_form_panel">
												<form action="" method="post" class="reply_double_form">
														<textarea rows="7" cols="30" class="reply_double_content"
																name="content"></textarea>
														<br> <input type="submit" value="回复"> <input
																type="reset" value="收起" class="hiddenReplyDoubleForm">
												</form>
										</div>
								</c:forEach>
						</div>
